 ...
 * the License.   package cn.afterturn.easypoi.excel.export.base;
 */
...
public abstract class BaseExportServiceTest extends ExportCommonService {
    ...
    private int createIndexCell(Row row, int index, ExcelExportEntity excelExportEntity) {
        if (excelExportEntity.getFormat() != null && excelExportEntity.getFormat().equals(PoiBaseConstants.IS_ADD_INDEX)) {
            createStringCell(row, 0, currentIndex + "",
                    index % 2 == 0 ? getStyles(false, null) : getStyles(true, null), null, null);
    ...
    public void createStringCell(Row row, int index, String text, CellStyle style,
                                 ExcelExportEntity entity) {
        createStringCell(row, index, text, style, entity, null);
    ...
    public void createStringCell(Row row, int index, String text, CellStyle style,
        ...
        //增加日期
        try {
            SimpleDateFormat dateFormat = new SimpleDateFormat(entity.getFormat());
            ...
            if (isDateFlag) {
                ...
                if (workbook != null) {
                    ...
                    cellStyle.setDataFormat(
                            workbook.getCreationHelper().createDataFormat().getFormat(entity.getFormat()));
                    style = cellStyle;
                }
    ...
    public void addStatisticsRow(CellStyle styles, Sheet sheet) {
        if (statistics.size() > 0) {
            ...
            createStringCell(row, 0, "合计", styles, null, null);
            for (Integer key : keys) {
                createStringCell(row, key, DOUBLE_FORMAT.format(statistics.get(key)), styles, null, null);
            }
            statistics.clear();
        }
